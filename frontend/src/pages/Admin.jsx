...
import { daysUntilExpiry, isMemberType, monthStartISO, peso, todayISO } from '../utils/format'
import {
  MEMBERSHIP_JOIN_FEE,
  DEFAULT_TYPE_PRICES,
...
import { buildTabPdf } from '../utils/salesPdf'

ChartJS.register(CategoryScale, LinearScale, BarElement, ArcElement, Tooltip, Legend)

const PAYMENT_METHOD_OPTIONS = ['Cash', 'GCash', 'Maya', 'Bank Transfer', 'Other']
...
function getPresetRange(preset) {
  ...
  switch (preset) {
    ...
    case 'thisWeek': {
      ...
      const startOfWeek = new Date(y, m, d - day + 1)
      return { from: fmt(startOfWeek), to: today }
    }
    case 'lastWeek': {
      const day = now.getDay() || 7
      const startOfLastWeek = new Date(y, m, d - day - 6)
      const endOfLastWeek = new Date(y, m, d - day)
      return { from: fmt(startOfLastWeek), to: fmt(endOfLastWeek) }
    }
    case 'thisMonth': return { from: fmt(new Date(y, m, 1)), to: today }
    case 'lastMonth': {
...
const REPORT_TAB_LABEL = Object.fromEntries(REPORT_TABS)
...
function enrollQualityColor(q) {
  if (q >= 70) return '#16a34a' // green
  if (q >= 45) return '#d97706' // amber
  return '#ef4444'              // red
}
...
const ALL_TYPE_OPTIONS = [
  ...
  ...memberTypeOptions.map(o => ({ label: 'M: ' + o.label, value: o.value })),
  ...nonMemberTypeOptions.map(o => ({ label: 'NM: ' + o.label, value: o.value })),
]

const ALL_PAYMENT_METHODS = ['All', 'Cash', 'GCash', 'Maya', 'Bank Transfer', 'Other']

const ALL_STATUS_OPTIONS = ['All', 'Paid', 'Not Paid']
...
export default function Admin() {
  ...
  const [pdfTab, setPdfTab] = useState('overview')
  ...
  const [editStaffConfirm, setEditStaffConfirm] = useState('')
  const [showEditStaffPassword, setShowEditStaffPassword] = useState(false)
  const [showEditStaffConfirm, setShowEditStaffConfirm] = useState(false)
  ...
  async function loadAuditLogs() {
    try {
      const data = await fetchAuditLogs({ from: dateRange.from, to: dateRange.to, role: auditRoleFilter, query: auditSearch, page: auditPage, limit: 50 })
      setAuditLogs(data.logs || [])
      setAuditTotalPages(data.totalPages || 1)
      setAuditTotal(data.total || 0)
      if (data.page) setAuditPage(data.page)
    } catch (err) {
      console.error(err)
    }
  }
  ...
  async function loadDashboard(overrides = {}) {
    try {
      const data = await fetchReportsDashboard({
        from: overrides.from || reportRange.from,
        to: overrides.to || reportRange.to,
  ...
  function switchReportTab(key) {
    ...
    if (key === 'transactions') {
      ...
      setReportRange({ from: today, to: today })
  ...
  async function handleEnrollFingerprint(member) {
    ...
    // If the USB/desk sensor is connected, enroll directly on it
  ...
  // Enroll directly on the USB/desk reader sensor
  ...
  async function confirmRegistration() {
    ...
    setSaving(true)
    try {
      if (confirmModal.kind === 'pdf') {
        await generateAndSavePdf()
        return
      }
      if (confirmModal.kind === 'staff-update') {
        const nextName = editStaffName.trim()
        await updateStaff(editingStaff.id, {
          username: nextName,
          password: editStaffPassword || undefined,
          confirmPassword: editStaffPassword ? editStaffConfirm : undefined,
        })
        setStatusMessage(`Staff account "${nextName}" updated successfully`)
      ...
      if (confirmModal.kind === 'non-member') {
        ...
        const saved = await createClient({
          ...
          sessionThreshold: clientSessionThreshold,
        })
        setReceipt({
  ...
  function openStaffUpdateConfirm() {
    if (!editingStaff) return
    const nextName = editStaffName.trim()
    if (!nextName) {
      setStatusMessage('Enter a username for the staff account')
      return
    }
    if (editStaffPassword && editStaffPassword !== editStaffConfirm) {
      setStatusMessage('New password and confirm password do not match')
      return
    }
    const nameChanged = nextName !== editingStaff.name
    if (!nameChanged && !editStaffPassword) {
      setStatusMessage('Enter a new username and/or password to update')
      return
    }
    const lines = [
      { label: 'Staff ID', value: editingStaff.staffId },
      { label: 'Current Username', value: editingStaff.name },
    ]
    if (nameChanged) lines.push({ label: 'New Username', value: nextName, strong: true })
    if (editStaffPassword) lines.push({ label: 'Password', value: editStaffPassword, strong: true })
    setConfirmModal({
      kind: 'staff-update',
      title: 'Confirm Staff Update',
      subtitle: `Review the credential changes before saving. ${editingStaff.name} will use them on their next sign in.`,
      lines,
    })
  }
  ...
  function buildReportFilterSummary() {
    if (!reportDash) return []
    return [
      { label: 'Report', value: 'Gym Reports Dashboard' },
      { label: 'Date Range', value: `${reportRange.from} to ${reportRange.to}` },
      { label: 'Payment Method', value: filterPaymentMethod === 'All' ? 'All Methods' : filterPaymentMethod },
      { label: 'Membership Type', value: filterMembershipType === 'All' ? 'All Types' : filterMembershipType },
      { label: 'Staff', value: filterStaffId === 'All' ? 'All Staff' : (reportDash.staffActivity.find(s => String(s.userId) === filterStaffId)?.name || 'All Staff') },
      { label: 'Status', value: filterStatus === 'All' ? 'All Statuses' : filterStatus },
      { label: 'Generated', value: new Date().toLocaleString() },
    ]
  }

  function handleSaveAsPdf(tab = 'membership-sales') {
    setPdfTab(tab)
    setConfirmModal({
      kind: 'pdf',
      title: 'Save Report as PDF',
      subtitle: 'A save dialog will open so you can choose the folder and file name.',
      lines: [
        { label: 'Report Page', value: REPORT_TAB_LABEL[tab] || tab },
        { label: 'Report Period', value: `${reportRange.from} to ${reportRange.to}` },
      ],
    })
  }
  ...
  async function generateAndSavePdf() {
    try {
      const doc = buildTabPdf(pdfTab, reportDash, reportRange)
      const blob = doc.output('blob')
      const tabLabel = (REPORT_TAB_LABEL[pdfTab] || 'Report').replace(/\s+/g, '')
      const suggestedName = `RDC-GYM-${tabLabel}_${reportRange.from}_to_${reportRange.to}.pdf`
      if (typeof window.showSaveFilePicker === 'function') {
        try {
          ...
          setStatusMessage(`PDF saved as "${handle.name}".`)
          return true
        } catch (err) {
          if (err?.name === 'AbortError') {
            setStatusMessage('PDF export cancelled - no file was saved.')
            return false
          }
          throw err
        }
      ...
      setStatusMessage('This browser cannot open a folder picker - the PDF was saved to your Downloads folder instead. Use Chrome or Edge to choose a location.')
      return true
    } catch (err) {
      console.error(err)
      setStatusMessage('Failed to generate the PDF. Please try again.')
      return false
    }
  }
  ...
  useEffect(() => {
    const before = () => document.body.classList.add('report-open')
    const after = () => document.body.classList.remove('report-open')
  ...
  }, [activePage, auditRoleFilter, dateRange, auditSearch, auditPage])

  useEffect(() => {
    if (activePage === 'Audit Logs') setAuditPage(1)
  }, [auditRoleFilter, dateRange, auditSearch])
  ...
  return (
    <div className="app-layout">
      <Sidebar items={adminItems} activeItem={activePage} onSelect={setActivePage} />
        ...
        <section className="content-card">
                        ...
                        <tr key={member.id ?? member.name}>
                          ...
                          <td className="action-cell">
                            ...
                            <button
                              ...
                              title={member.fingerprint_id ? 'Re-enroll fingerprint' : 'Enroll fingerprint'}
          ...
          {activePage === 'Manage Staff' && (
            <>
              ...
              <div className="submit-row" style={{ justifyContent: 'flex-start', marginTop: 12, marginBottom: 12 }}>
                <button type="button" className="pill active" onClick={handleAddStaff}>+ Add new staff account</button>
              </div>
              {editingStaff && (
                <div className="register-card staff-edit-card">
                  <div className="section-title-row" style={{ marginBottom: 16 }}>
                    <div>
                      <h3>Edit Staff</h3>
                      <p className="section-subtitle">Update the username and/or password for {editingStaff.name} ({editingStaff.staffId}). Leave the password fields empty to keep the current password.</p>
                    </div>
                    <button type="button" className="pill" onClick={handleCancelEditStaff}>Cancel Edit</button>
                  </div>
                  <div className="form-grid staff-form">
                    <label>
                      Username
                      <input value={editStaffName} onChange={e => setEditStaffName(e.target.value)} placeholder="Enter new username" maxLength={100} />
                    </label>
                    <label>
                      ...
                      <div className="password-field">
                        ...
                        <button
                          type="button"
                          className={showEditStaffPassword ? 'password-toggle show' : 'password-toggle'}
                          onClick={() => setShowEditStaffPassword(prev => !prev)}
                          aria-label={showEditStaffPassword ? 'Hide password' : 'Show password'}
                        >
                          <span className="eye-icon" aria-hidden="true"></span>
                        </button>
                      </div>
                    ...
                    <label>
                      ...
                      <div className="password-field">
                        <input
                          type={showEditStaffConfirm ? 'text' : 'password'}
                          value={editStaffConfirm}
                          onChange={e => setEditStaffConfirm(e.target.value)}
                          placeholder="Re-enter new password"
                        />
                        <button
                          type="button"
                          className={showEditStaffConfirm ? 'password-toggle show' : 'password-toggle'}
                          onClick={() => setShowEditStaffConfirm(prev => !prev)}
                          aria-label={showEditStaffConfirm ? 'Hide confirm password' : 'Show confirm password'}
                        >
                          <span className="eye-icon" aria-hidden="true"></span>
                        </button>
                      </div>
                  ...
                  <div className="submit-row" style={{ justifyContent: 'flex-start' }}>
                    <button type="button" className="pill action" onClick={openStaffUpdateConfirm}>Save Changes</button>
                    <button type="button" className="pill danger" onClick={handleCancelEditStaff}>Cancel</button>
                  </div>
                </div>
              )}
              <div className="table-card">
                  ...
                  <tbody>
                    {staffAccounts.map(staff => (
                      <tr key={staff.staffId}>
                        ...
                        <td>
                          <button className="pill" onClick={() => handleEditStaff(staff)}>Edit</button>
            ...
            <>
              ...
              <div className="audit-pagination-bar">
                <span className="audit-page-info">
                  {auditTotal > 0 ? `Showing ${(auditPage - 1) * 50 + 1}–${Math.min(auditPage * 50, auditTotal)} of ${auditTotal} events` : 'No events'}
                </span>
                <div className="audit-page-controls">
                  <button type="button" className="pill" disabled={auditPage <= 1} onClick={() => setAuditPage(p => p - 1)}>Back</button>
                  <span className="audit-page-number">Page {auditPage} of {auditTotalPages}</span>
                  <button type="button" className="pill" disabled={auditPage >= auditTotalPages} onClick={() => setAuditPage(p => p + 1)}>Next</button>
                </div>
              </div>
            ...
            <div className="report-dashboard">
              <div className="section-title-row header-right-label no-print">
                <div>
                  ...
                  <p className="section-subtitle">Comprehensive analytics and insights for your gym.</p>
                </div>
              ...
              <div className="report-nav no-print">
                {REPORT_TABS.map(([key, label]) => (
                  <button key={key} type="button" className={`report-nav-btn ${reportTab === key ? 'active' : ''}`} onClick={() => switchReportTab(key)}>{label}</button>
                ))}
              </div>
              ...
              <div className="report-filters no-print">
                <div className="report-filter-row">
                  ...
                  <label>
                    Membership
                    <select value={filterMembershipType} onChange={e => setFilterMembershipType(e.target.value)}>
                      {ALL_TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                    </select>
                  </label>
                  <label>
                    Staff
                    <select value={filterStaffId} onChange={e => setFilterStaffId(e.target.value)}>
                      ...
                      {(reportDash?.staffActivity || []).map(s => <option key={s.userId} value={s.userId}>{s.name}</option>)}
                    </select>
                  </label>
                  <label>
                    Status
                  ...
                  <button type="button" className="pill action" onClick={() => loadDashboard()}>Apply Filter</button>
                  <button type="button" className="pill" onClick={() => {
                    setReportRange(getPresetRange('thisMonth'))
                ...
                <div className="report-filter-dates">
                  <span className="report-range-label">{reportRange.from} to {reportRange.to}</span>
                </div>
              </div>
                ...
                <>
                  ...
                  <div className="report-section">
                    ...
                    <div className="report-summary-grid">
                      <div className="report-summary-card blue"><div className="report-card-label">Today's Revenue</div><div className="report-card-value">{peso(reportDash.summary.todayRevenue)}</div></div>
                      <div className="report-summary-card green"><div className="report-card-label">Total Revenue</div><div className="report-card-value">{peso(reportDash.summary.totalRevenue)}</div></div>
                      <div className="report-summary-card purple"><div className="report-card-label">Today's Transactions</div><div className="report-card-value">{reportDash.summary.todayTransactions}</div></div>
                      <div className="report-summary-card orange"><div className="report-card-label">Total Transactions</div><div className="report-card-value">{reportDash.summary.totalTransactions.toLocaleString()}</div></div>
                      <div className="report-summary-card teal"><div className="report-card-label">Active Members</div><div className="report-card-value">{reportDash.summary.activeMembers}</div></div>
                      <div className="report-summary-card cyan"><div className="report-card-label">Today's Check-Ins</div><div className="report-card-value">{reportDash.summary.todayCheckIns}</div></div>
                      <div className="report-summary-card pink"><div className="report-card-label">Today's Check-Outs</div><div className="report-card-value">{reportDash.summary.todayCheckOuts}</div></div>
                      <div className="report-summary-card red"><div className="report-card-label">Currently Inside</div><div className="report-card-value">{reportDash.summary.currentlyInside}</div></div>
                    </div>
                  </div>
                  <div className="report-actions no-print">
                    <button type="button" className="pill action" onClick={() => handleSaveAsPdf('overview')}>Export PDF</button>
                    <button type="button" className="pill" onClick={handlePrintReport}>Print</button>
                  </div>
                  </>
                  )}

                  {reportTab === 'membership-sales' && (
                  <>
                  <div className="report-section">
                    <h4>Membership Sales</h4>
                    <div className="report-revenue-summary">
                      <span>Most Sold: <strong>{reportDash.membershipSales.items.length > 0 ? reportDash.membershipSales.items.reduce((a, b) => Number(a.count) > Number(b.count) ? a : b).type : 'N/A'}</strong></span>
                      <span>Highest Revenue: <strong>{reportDash.membershipSales.items.length > 0 ? reportDash.membershipSales.items.reduce((a, b) => Number(a.revenue) > Number(b.revenue) ? a : b).type : 'N/A'}</strong></span>
                    </div>
                    <div className="report-table-container">
                      <table className="report-table">
                        <thead><tr><th>Membership Type</th><th>Number Sold</th><th>Revenue</th><th>% of Sales</th></tr></thead>
                        <tbody>
                          {reportDash.membershipSales.items.length === 0 && <tr><td colSpan={4}>No membership sales in this period.</td></tr>}
                          {reportDash.membershipSales.items.map(row => (
                            <tr key={row.type}><td>{row.type}</td><td>{row.count}</td><td>{peso(row.revenue)}</td><td>{row.percentage}%</td></tr>
                          ))}
                        </tbody>
                      </table>
                    </div>
                  ...
                  <div className="report-actions no-print">
                    <button type="button" className="pill action" onClick={() => handleSaveAsPdf('membership-sales')}>Export PDF</button>
                    <button type="button" className="pill" onClick={handlePrintReport}>Print</button>
                  </div>
                  </>
                  )}
                  ...
                  <div className="report-section">
                    ...
                    <div className="report-summary-grid four">
                      <div className="report-summary-card cyan"><div className="report-card-label">Total Check-Ins</div><div className="report-card-value">{reportDash.attendance.totalCheckIns.toLocaleString()}</div></div>
                      <div className="report-summary-card pink"><div className="report-card-label">Total Check-Outs</div><div className="report-card-value">{reportDash.attendance.totalCheckOuts.toLocaleString()}</div></div>
                      <div className="report-summary-card red"><div className="report-card-label">Currently Inside</div><div className="report-card-value">{reportDash.attendance.currentlyInside}</div></div>
                      <div className="report-summary-card blue"><div className="report-card-label">Total Visits</div><div className="report-card-value">{reportDash.attendance.totalVisits.toLocaleString()}</div></div>
                    </div>
                    {reportDash.attendance.dailyVisits.length > 0 && (
                      <div className="report-chart-container" style={{ marginTop: 16 }}>
                        <Bar data={{
                          labels: reportDash.attendance.dailyVisits.map(d => d.day),
                          datasets: [{ label: 'Visits', data: reportDash.attendance.dailyVisits.map(d => d.visits), backgroundColor: 'rgba(99,102,241,0.7)' }],
                        }} options={{ responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true } }, plugins: { legend: { display: false } } }} />
                      </div>
                    )}
                  </div>
                  <div className="report-actions no-print">
                    <button type="button" className="pill action" onClick={() => handleSaveAsPdf('attendance')}>Export PDF</button>
                    <button type="button" className="pill" onClick={handlePrintReport}>Print</button>
                  </div>
                  </>
                  )}

                  {reportTab === 'peak-hours' && (
                  <>
                  <div className="report-section">
                    <div className="report-section-header">
                      <h4>Peak Gym Hours</h4>
                      <span className="report-peak-badge">Peak Hour: <strong>{reportDash.peakHour}</strong></span>
                    </div>
                    {reportDash.peakHours.length > 0 ? (
                      <div className="report-chart-container">
                        <Bar data={{
                          labels: reportDash.peakHours.map(h => h.label),
                          datasets: [{ label: 'Visits', data: reportDash.peakHours.map(h => h.visits), backgroundColor: reportDash.peakHours.map(h => h.visits === Math.max(...reportDash.peakHours.map(x => x.visits)) ? 'rgba(239,68,68,0.8)' : 'rgba(99,102,241,0.5)') }],
                        }} options={{ responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true } }, plugins: { legend: { display: false } } }} />
                      </div>
                    ) : <p className="report-empty">No attendance data for this period.</p>}
                  </div>
                  <div className="report-actions no-print">
                    <button type="button" className="pill action" onClick={() => handleSaveAsPdf('peak-hours')}>Export PDF</button>
                    <button type="button" className="pill" onClick={handlePrintReport}>Print</button>
                  </div>
                  </>
                  )}

                  {reportTab === 'membership-status' && (
                  <>
                  <div className="report-section">
                    ...
                    <div className="report-chart-row">
                      <div className="report-chart-container small">
                        <Doughnut data={{
                          ...
                          datasets: [{ data: [reportDash.membershipStatus.active, reportDash.membershipStatus.expiringSoon, reportDash.membershipStatus.expired], backgroundColor: ['#22c55e', '#f59e0b', '#ef4444'] }],
                        }} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right' } } }} />
                      </div>
                      <div className="report-membership-status-list">
                        <div className="report-status-item"><span className="report-status-dot green" /> Active: <strong>{reportDash.membershipStatus.active}</strong></div>
                        <div className="report-status-item"><span className="report-status-dot yellow" /> Expiring Soon: <strong>{reportDash.membershipStatus.expiringSoon}</strong></div>
                        <div className="report-status-item"><span className="report-status-dot red" /> Expired: <strong>{reportDash.membershipStatus.expired}</strong></div>
                      </div>
                    </div>
                  </div>
                  <div className="report-actions no-print">
                    <button type="button" className="pill action" onClick={() => handleSaveAsPdf('membership-status')}>Export PDF</button>
                    <button type="button" className="pill" onClick={handlePrintReport}>Print</button>
                  </div>
                  </>
                  )}

                  {reportTab === 'staff-activity' && (
                  <>
                  <div className="report-section">
                    <div className="report-section-header">
                      <h4>Staff Activity</h4>
                      <select value={filterStaffId} onChange={e => setFilterStaffId(e.target.value)} className="report-staff-filter">
                        <option value="All">All Staff</option>
                        {reportDash.staffActivity.map(s => <option key={s.userId} value={s.userId}>{s.name}</option>)}
                      </select>
                    </div>
                    <div className="report-table-container">
                      <table className="report-table">
                        <thead><tr><th>Staff Name</th><th>Transactions</th><th>Sales</th><th>Check-Ins</th><th>Check-Outs</th></tr></thead>
                        <tbody>
                          {reportDash.staffActivity.filter(s => filterStaffId === 'All' || String(s.userId) === filterStaffId).length === 0 && <tr><td colSpan={5}>No staff activity data.</td></tr>}
                          {reportDash.staffActivity.filter(s => filterStaffId === 'All' || String(s.userId) === filterStaffId).map(s => (
                            <tr key={s.userId}><td>{s.name}</td><td>{s.transactions}</td><td>{peso(s.sales)}</td><td>{s.checkIns}</td><td>{s.checkOuts}</td></tr>
                          ))}
                        </tbody>
                      </table>
                    </div>
                  ...
                  <div className="report-actions no-print">
                    <button type="button" className="pill action" onClick={() => handleSaveAsPdf('staff-activity')}>Export PDF</button>
                    <button type="button" className="pill" onClick={handlePrintReport}>Print</button>
                  </div>
                  </>
                  )}
                  ...
                  <div className="report-section">
                    ...
                    <div className="report-table-container">
                      <table className="report-table">
                        <thead><tr><th>Transaction #</th><th>Date</th><th>Time</th><th>Member</th><th>Type</th><th>Amount</th><th>Payment Method</th><th>Status</th></tr></thead>
                        <tbody>
                          {reportDash.recentTransactions.length === 0 && <tr><td colSpan={8}>No transactions found.</td></tr>}
                          {reportDash.recentTransactions.map(tx => (
                            <tr key={tx.transactionNo}>
                              ...
                              <td>{tx.member}</td>
                              <td>{tx.type}</td>
                              <td>{peso(tx.amount)}</td>
                              <td>{tx.paymentMethod}</td>
                              <td><span className="report-badge paid">{tx.status}</span></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="report-actions no-print">
                    <button type="button" className="pill action" onClick={() => handleSaveAsPdf('transactions')}>Export PDF</button>
                    <button type="button" className="pill" onClick={handlePrintReport}>Print</button>
                  </div>
                  </>
                  )}
